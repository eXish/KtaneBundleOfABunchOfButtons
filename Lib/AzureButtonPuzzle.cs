...
namespace BlueButtonLib
{
    public class AzureButtonPuzzle
    {
        ...
        private static readonly Coord[][] _allArrows;
        ...
        static AzureButtonPuzzle()
        {
            ...
            IEnumerable<Coord[]> generateArrows(Coord[] sofar)
            {
                ...
                var start = sofar.Length == 0 ? new Coord(4, 4, 0, 0) : sofar.Last();
                for (var dir = 0; dir < 8; dir++)
                {
                    var next = start.AddWrap(dxs[dir], dys[dir]);
                    if (next.Index != 0 && !sofar.Contains(next))
                        foreach (var result in generateArrows(insert(sofar, sofar.Length, next)))
                            yield return result;
                }
            }
            _allArrows = generateArrows(new Coord[0]).ToArray();
        }
        ...
        public static AzureButtonPuzzle Generate(int seed)
        {
            ...
            var allArrows = sol.Select((ltr, ltrIx) => _allArrows.Where(ar => Enumerable.Range(0, ar.Length).All(arIx => grid[ar[arIx].AddWrap(ltrIx, ltrIx).Index] == ((ltr - 'A' + 1) / _powersOf3[2 - arIx]) % 3)).ToArray()).ToArray();
            var allLetterData = sol
                .SelectMany((ltr, solLtrIx) => allArrows[solLtrIx].Select(ar => (solLtrIx, coords: ar, ltrs: Enumerable.Range(1, 3).Select(disp => (char) (ar.Select(c => grid[c.AddWrap(solLtrIx + disp, solLtrIx + disp).Index]).Aggregate(0, (p, n) => p * 3 + n) + 'A' - 1)).JoinString())))
            ...
            foreach (var tabooLetter in Enumerable.Range(0, 26).Select(x => (char) ('A' + x)).Where(x => !sol.Contains(x)).ToArray().Shuffle(rnd))
            {
                ...
                while (arrows.Contains(null))
                {
                    ...
                    var (solLtrIx, coords, ltrs) = letterData[candidateIx];
                    for (var i = 0; i < ltrs.Length; i++)
                        if (ltrs[i] == tabooLetter)
                            tabooFilled[i] = true;
                    arrows[solLtrIx] = coords;
                ...
                for (var disp = 0; disp < 4; disp++)
                    otherWords.Add(arrows.Select((ar, arIx) => (char) (ar.Select(c => grid[c.AddWrap(arIx + disp, arIx + disp).Index]).Aggregate(0, (p, n) => 3 * p + n) + 'A' - 1)).JoinString());
