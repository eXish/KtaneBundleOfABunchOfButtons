...
namespace BlueButtonLib
{
    public class AzureButtonPuzzle
    {
        ...
        private static readonly (Coord coord, int dir)[][] _allArrows;

        static AzureButtonPuzzle()
        {
            ...
            IEnumerable<(Coord coord, int dir)[]> generateArrows((Coord coord, int dir)[] sofar)
            {
                ...
                for (var dir = 0; dir < 8; dir++)
                {
                    ...
                    if (next.Index != 0 && !sofar.Any(tup => tup.coord == next))
                        foreach (var result in generateArrows(insert(sofar, sofar.Length, (next, dir))))
                            yield return result;
                }
            }
            _allArrows = generateArrows(new (Coord coord, int dir)[0]).ToArray();
        }
        ...
        public static AzureButtonPuzzle Generate(int seed)
        {
            ...
            var allArrows = sol.Select((ltr, ltrIx) => _allArrows.Where(ar => Enumerable.Range(0, ar.Length).All(arIx => grid[ar[arIx].coord.AddWrap(ltrIx, ltrIx).Index] == ((ltr - 'A' + 1) / _powersOf3[2 - arIx]) % 3)).ToArray()).ToArray();
            var allLetterData = sol
                .SelectMany((ltr, solLtrIx) => allArrows[solLtrIx].Select(ar => (solLtrIx, arrowTups: ar, ltrs: Enumerable.Range(1, 3).Select(disp => (char) (ar.Select(tup => grid[tup.coord.AddWrap(solLtrIx + disp, solLtrIx + disp).Index]).Aggregate(0, (p, n) => p * 3 + n) + 'A' - 1)).JoinString())))
            ...
            foreach (var tabooLetter in Enumerable.Range(0, 26).Select(x => (char) ('A' + x)).Where(x => !sol.Contains(x)).ToArray().Shuffle(rnd))
            {
                ...
                while (arrows.Any(a => a == null))
                {
                    ...
                    var (solLtrIx, arrowTups, ltrs) = letterData[candidateIx];
                    for (var i = 0; i < ltrs.Length; i++)
                        if (ltrs[i] == tabooLetter)
                            tabooFilled[i] = true;
                    arrows[solLtrIx] = arrowTups;
                ...
                for (var disp = 0; disp < 4; disp++)
                    otherWords.Add(arrows.Select((ar, arIx) => (char) (ar.Select(tup => grid[tup.coord.AddWrap(arIx + disp, arIx + disp).Index]).Aggregate(0, (p, n) => 3 * p + n) + 'A' - 1)).JoinString());
                ...
                return new AzureButtonPuzzle
                {
                    ...
                    Arrows = arrows.Select(ar => ar.Select(a => a.coord).ToArray()).ToArray(),
                    ArrowDirs = arrows.Select(ar => ar.Select(a => a.dir).ToArray()).ToArray()
                };
